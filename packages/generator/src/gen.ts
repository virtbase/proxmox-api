...
const protectFieldName = (name: string) => {
  if (~name.indexOf("-") || ~name.indexOf(".")) return `'${name}'`;
  return name;
};

const snakeToLowerCamel = (str: string) => {
  const lowercameCase = str.replace(/([-_ ][a-z])/g, (group) =>
    group.toUpperCase().replace(" ", "").replace("-", "").replace("_", ""),
  );
  return lowercameCase;
};

const snakeToUpperCamel = (str: string) => {
  const lowercameCase = snakeToLowerCamel(str);
  return lowercameCase[0].toUpperCase() + lowercameCase.substr(1);
};

const unNify = (propName: string): string[] => {
  if (!~propName.indexOf("[n]")) return [protectFieldName(propName)];
  let max = 4;
...
`;
  ...
  appendCommentLines(dest: string[], lineOffset: string, ...lines: string[]) {
    for (const doc of lines) {
      let line = doc.trim();
      if (!doc.startsWith("@")) line = line.replace(/([{}])/g, "\\$1");
  ...
  mapType2(pveType: PveCallParameters, name: string): string {
    ...
    if (pveType.type === "string" && pveType.enum) {
      declaration.push(
        `${TAB}export type ${newType} = '${pveType.enum.sort().join("' | '")}';`,
      );
  ...
  generate(node: pveApiNode, lineOffset0: string, pathVariables: Set<string>) {
    ...
    if (text.startsWith("{")) {
      ...
      if (varname === "vmid") pathParamType = "number";
    ...
    if (info) {
      for (const mtd of Object.keys(info)) {
        ...
        if (typeof theInfo.permissions !== "undefined") {
          this.code.push(
            `${lineOffset} * @permissions ${JSON.stringify(theInfo.permissions)}`,
          );
        }
        ...
        if (properties) {
          for (const propName of Object.keys(properties)) {
            if (pathVariables.has(propName)) continue;
        ...
        //    debugger;
        const returnType = this.mapsReturn(node, theInfo, path);
        this.code.push(
          `${lineOffset}$${mtd.toLowerCase()}(${pTxt}): Promise<${this.mapType(returnType)}>;`,
        );
      }
    }
    if (children) {
  ...
  genModelObject(
    info: PveParametersObject | PveParametersUndef,
    lineOffset0: string,
    additionalProperties?: 0 | 1,
  ): string {
    if (!info.properties) return "any"; // no data ...
    const lines: string[] = [];
    lines.push("{");
    const lineOffset = lineOffset0 + TAB;
  ...
  mapsReturn(node: pveApiNode, theInfo: PveCallDesc, path: string): string {
    ...
    let retTypeOptfix = "";
    let typeName = snakeToLowerCamel(
      path
        .replace(/{[a-z]+}/g, "")
        .replace(/\/\//g, " ")
        .replace(/\//g, " ")
    ...
    if (fullType === "any") {
      ...
      this.appendComment(
        code,
        TAB,
        `Returned by ${theInfo.method} ${node.path}`,
      );
